...
def run_app():
    ...
    model1 = joblib.load('model1.joblib')  # Load the trained model
    scaler1 = joblib.load('scaler1.joblib')  # Load the fitted scaler
    ...
    if st.button(labels[lang_code]['Submit']):
        ...
        scaled_data = scaler1.transform([list(data.values())])

        # Make prediction using the trained model
        prediction = model1.predict(scaled_data)[0]
